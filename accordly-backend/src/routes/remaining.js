...
const { sendCoparentInviteEmail } = require('../services/email');
...
r6.post('/relationships', async (req, res, next) => {
  try {
    ...
    const inviterName = `${req.user.first_name || ''} ${req.user.last_name || ''}`.trim() || 'Your co-parent';
    const signupLink = `${process.env.APP_URL || 'https://www.accordlyparent.com'}/early-access/accordly-login`;
    sendCoparentInviteEmail(invite_email, inviterName, signupLink)
      .catch(err => logger.error('Invite email failed:', err.message));

    res.status(201).json({ relationship: result.rows[0] });
  } catch(e){next(e)}
});
r6.post('/relationships/:id/resend-invite', async (req, res, next) => {
  try {
    const { id } = req.params;

    const rel = await q6(
      `SELECT * FROM coparent_relationships
       WHERE id = $1 AND (parent_a_id = $2 OR parent_b_id = $2)`,
      [id, req.user.id]
    );
    if (!rel.rows.length) return res.status(404).json({ error: 'Relationship not found.' });

    const relationship = rel.rows[0];
    if (relationship.parent_b_id) {
      return res.status(400).json({ error: 'This co-parent has already joined — nothing to resend.' });
    }
    if (!relationship.invite_email) {
      return res.status(400).json({ error: 'No invite email on file for this relationship.' });
    }

    const inviterName = `${req.user.first_name || ''} ${req.user.last_name || ''}`.trim() || 'Your co-parent';
    const signupLink = `${process.env.APP_URL || 'https://www.accordlyparent.com'}/early-access/accordly-login`;
    const result = await sendCoparentInviteEmail(relationship.invite_email, inviterName, signupLink);

    if (!result.sent) {
      return res.status(502).json({ error: 'Could not send the invite email. Please try again shortly.' });
    }

    res.json({ success: true, message: `Invite resent to ${relationship.invite_email}.` });
  } catch(e){next(e)}
});
